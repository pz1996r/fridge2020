...
router.post('/', async (req, res) => {
    ...
    return addProduct(req.body, res);
})

router.get('/:product', async (req, res) => {
    const product = !Number.isNaN(Number(req.params.product)) ? await Product.findOne({ id: req.params.product }).populate('units', 'name -_id').select('id name type units -_id') : await Product.findOne({ name: req.params.product }).populate('units', 'name -_id').select('id name type units -_id');
